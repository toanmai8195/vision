...
- [x] Tạo `agent/pipeline/nodes/planner.py` — gọi Ollama, trả plan
...
- [x] Tạo `agent/pipeline/nodes/reviewer.py` — Ollama review code
...
### Phase 5 — k3s + Ollama in Cluster → [phases/phase5.md](phases/phase5.md)
- [ ] Cài k3s: `curl -sfL https://get.k3s.io | sh`
- [ ] Tạo `agent/k8s/namespace.yaml` — namespace `vision-agents`
- [ ] Tạo `agent/k8s/ollama-deployment.yaml` — Ollama chạy trong cluster
- [ ] Tạo `agent/k8s/ollama-service.yaml` — ClusterIP :11434
- [ ] Tạo `agent/k8s/workspace-pvc.yaml` — PVC 20Gi (local-path)
- [ ] Cập nhật `.scion/settings.yaml` — thêm `runtime: kubernetes`
- [ ] Cập nhật `agent/agents/*.yaml` — `OLLAMA_BASE_URL` trỏ vào service cluster
- [ ] `kubectl apply -f agent/k8s/`
- [ ] Pull model vào Ollama trong cluster: `kubectl exec deploy/ollama -- ollama pull qwen2.5-coder:7b`
- [ ] Test: `kubectl get pods -n vision-agents` thấy ollama pod + SCION agent pods
- [ ] Test: pipeline chạy end-to-end trên k3s

### Phase 6 — GitHub Actions CI/CD → [phases/phase6.md](phases/phase6.md)
- [ ] Tạo `agent/k8s/gh-runner-deployment.yaml` — self-hosted runner trên k3s
...
- [ ] Test: CI fail → Ollama implementer nhận lỗi, retry
